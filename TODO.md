...
## A generated file that was committed, and leaked — 2026-09-01

`doc/canger.1` is generated by `canger --man`, and it was also checked in. Nothing compared the
two, and it went three releases without being regenerated: the committed page was headed
`canger 0.3.0` while the program rendered 0.6.0, and it described neither removable drives nor
version control. `build.sh dist` copied it straight into both tarballs, so anyone unpacking one
read a manual three releases old. The `.deb` was right only because it happened to generate its own.

**Then regenerating it made things worse, which is the part worth remembering.** `./canger.sh --man`
reads `~/.config/canger` on the way past, and `--man` renders the bindings and commands that are
*actually loaded*. So the regenerated page documented the maintainer's own `efc`, `fzf_locate` and
`file_convert_text` instead of the defaults — a private configuration about to be shipped to
everyone. Caught by a test written for the staleness, which failed for a different reason than it
was aimed at.

The `.deb` had been doing this all along: `"$lib/canger" --man` with the user's configuration in
place. Measured on the 0.6.0 package built an hour earlier — one mention of `efc` in the manual it
installs. The tarballs were clean, only because they copied the checked-in file rather than
generating.

Three changes:

- **Both generation sites pass `--clean`**, so the manual documents the program rather than
  whoever built it.
- **`doc/canger.1` is no longer committed.** Nothing consumed it: `dist` generates it into the
  staging directory, the `.deb` generates its own, and `?` then `m` renders one into a temporary
  file at runtime. A generated file kept under version control had already produced two defects;
  deleting it removes the class rather than the instance.
- The test written to pin the committed copy went with it. The remaining `ManPageTests` assert the
  structure of what is generated, which is the part that can actually regress.

Verified on rebuilt artifacts: no mention of `efc`, `fzf_locate` or `file_convert_text` in either
